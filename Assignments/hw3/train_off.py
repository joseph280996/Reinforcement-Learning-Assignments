...
class NStepOffPolicySarsa:
    ...
    def calculate_importance_ratio(self, t: int) -> float:
        """
        Calculate importance sampling ratio only for best action
        ρ = π(a|s) / b(a|s) where a is the greedy action
        """
        
        S_t = self.S[t]

        best_action = self.get_greedy_action(S_t)
        
        # Behavior policy probability for best action
        behavior_probs = self.get_behavior_policy_prob(S_t)
        behavior_prob = behavior_probs[best_action]
        
        return 1 / behavior_prob
    ...
    def sophisticated_update(self, tau: int) -> None:
        ...
        h = tau + self.n
        G = self.calculate_truncated_return(tau, min(h, self.T))

        # Update Q-value using (7.2)
        self.Q[tuple(self.S[tau])] += self.alpha * (G - self.Q[tuple(self.S[tau])])
    ...
    def simplified_update(self, tau: int) -> None:
        ...
        self.Q[tau_state] += (
            self.alpha
            * np.prod(np.array(self.rho)[tau : min(tau + self.n, self.T - 1) + 1], axis = 0)
            * (G - self.Q[tau_state])
        )
